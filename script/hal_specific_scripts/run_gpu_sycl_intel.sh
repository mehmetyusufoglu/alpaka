...
# Function to set up the environment for gpu-sycl-intel
setup_environment_gpu_sycl_intel() {
    echo "Setting up environment for gpu-sycl-intel..."
    ...
    spack load /lzqeu6m || { echo "Failed to load oneapi compilers"; return 1; }  # Specific hash for an intel compiler
    spack load intel-oneapi-dpl@2022.2.0 || { echo "Failed to load Intel oneAPI DPL"; return 1; }

    # Verify if icpx (Intel C++ Compiler) exists
    which icpx > /dev/null || { echo "Error: icpx not found. Ensure Intel oneAPI compilers are loaded properly."; return 1; }

    echo "Environment setup completed successfully."
}

# Function to configure, build, and run the benchmark for gpu-sycl-intel
build_and_run_gpu_sycl_intel() {
    local preset="gpu-sycl-intel"
    ...
    extra_flags="-Dalpaka_ACC_SYCL_ENABLE=ON \
...
# Step 3: Set up environment for gpu-sycl-intel
if ! setup_environment_gpu_sycl_intel; then
    echo "Failed to set up the environment. Exiting."
    exit 1
fi

# Step 4: Configure, build, and run for gpu-sycl-intel
build_and_run_gpu_sycl_intel
