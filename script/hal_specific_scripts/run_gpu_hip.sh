...
build_and_run_gpu_hip() {
    ...
    # Run the benchmark using Slurm
    echo "Submitting BabelStream benchmark to Slurm..."
    sbatch <<EOF
#!/bin/bash
#SBATCH --job-name=babelstream-benchmark
#SBATCH --output=$results_file
#SBATCH --error=$results_dir/babelstream-error-%j.txt
...
# Load necessary modules or environment variables (if required)
...
# Run the benchmark
echo "Running BabelStream benchmark..."
"\$babelstream_executable" --array-size=33554432 --number-runs=100
