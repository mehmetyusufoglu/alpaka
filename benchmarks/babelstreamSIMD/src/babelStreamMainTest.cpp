...
public:
    ALPAKA_NO_HOST_ACC_WARNING
        ...
        ALPAKA_FN_ACC auto operator()(TAcc const& acc, T const* a, T* c) const {
        ...
        constexpr size_t simdWidth = PortableSimd<T, TAcc>::size();

        PortableSimd<T, TAcc> simd_data_a;
        simd_data_a.load(&a[globalIdx * simdWidth]);

        PortableSimd<T, TAcc> simd_data_c = simd_data_a;
        simd_data_c.store(&c[globalIdx * simdWidth]);
    }
};
...
public:
        ...
        ALPAKA_FN_ACC void operator()(TAcc const& acc, T* b, T const* c) const
    {
        ...
        PortableSimd<T, TAcc> simd_data_c;
        simd_data_c.load(&c[globalIdx * simdWidth]);

        const PortableSimd<T, TAcc> scalar(static_cast<T>(scalarVal));

        PortableSimd<T, TAcc> simd_data_b = simd_data_c * scalar;
        simd_data_b.store(&b[globalIdx * simdWidth]);
    }
};
...
public:
        ...
        ALPAKA_FN_ACC void operator()(TAcc const& acc, T const* a, T const* b, T* c) const
    {
        ...
        PortableSimd<T, TAcc> simd_data_a;
        PortableSimd<T, TAcc> simd_data_b;
        simd_data_a.load(&a[globalIdx * simdWidth]);
        simd_data_b.load(&b[globalIdx * simdWidth]);

        PortableSimd<T, TAcc> simd_data_c = simd_data_a + simd_data_b;
        simd_data_c.store(&c[globalIdx * simdWidth]);
    }
};
...
public:
        ...
        ALPAKA_FN_ACC void operator()(TAcc const& acc, T* a, T const* b, T const* c) const
    {
        ...
        PortableSimd<T, TAcc> simd_data_a = simd_data_b + (simd_data_c * scalar);
        simd_data_a.store(&a[globalIdx * simdWidth]);
    }
};
...
//         tbSum[local_i] = PortableSimd<T, TAcc>(0);

//                // Perform SIMD dot product
//         for(; i < arraySize; i += totalThreads * simdWidth)
//         {
//             PortableSimd<T, TAcc> simd_data_a;
//             PortableSimd<T, TAcc> simd_data_b;
//             simd_data_a.load(&a[i]);
//             simd_data_b.load(&b[i]);

//             tbSum[local_i] += simd_data_a * simd_data_b;
//         }

//                // Sum up the results in shared memory
//         alpaka::syncBlockThreads(acc);
//         auto const blockSize = alpaka::getWorkDiv<alpaka::Block, alpaka::Threads>(acc)[0];

//         for(Idx offset = blockSize*simdWidth / 2; offset > 0; offset /= 2)
//         {
//             if(local_i < offset)
//             {
//                 tbSum[local_i] += tbSum[local_i + offset];
...
//                // Store the result in the global sum array
//                 auto const gridBlockIndex = alpaka::getIdx<alpaka::Grid, alpaka::Blocks>(acc)[0];
//                 if(local_i == 0)
//                     sum[gridBlockIndex] = tbSum[local_i];
//             }
...
void testKernels()
{
    ...
    constexpr size_t simdWidth = PortableSimd<DataType, Acc>::size();
    alpaka::KernelCfg<Acc> const kernelCfgSIMD = {Vec::all(arraySize/simdWidth), elementsPerThread};
    ...
    auto const workDivCopy
        = alpaka::getValidWorkDiv(kernelCfgSIMD, devAcc, CopyKernel<DataType>(), bufAccInputAPtr, bufAccInputBPtr);
    auto const workDivMult
        = alpaka::getValidWorkDiv(kernelCfgSIMD, devAcc, MultKernel<DataType>(), bufAccInputAPtr, bufAccInputBPtr);
    auto const workDivAdd
        = alpaka::getValidWorkDiv(kernelCfgSIMD, devAcc, AddKernel<DataType>(), bufAccInputAPtr, bufAccInputBPtr, bufAccOutputCPtr);

    auto const workDivTriad = alpaka::getValidWorkDiv(
        kernelCfgSIMD,
        devAcc,
        TriadKernel<DataType>(),
        bufAccInputAPtr,
        bufAccInputBPtr,
        bufAccOutputCPtr);
    ...
    for(auto i = 0; i < numberOfRuns; i++)
    {
        if(kernelsToBeExecuted == KernelsToRun::All)
        {
            ...
            measureKernelExec(
                [&]() { alpaka::exec<Acc>(queue, workDivCopy, CopyKernel<DataType>(), bufAccInputAPtr, bufAccOutputCPtr); },
                "CopyKernel");

            // Test the scaling-kernel. Calculate B=scalar*C. Where C = A.
            measureKernelExec(
                [&]() { alpaka::exec<Acc>(queue, workDivMult, MultKernel<DataType>(), bufAccInputBPtr, bufAccOutputCPtr); },
                "MultKernel");
